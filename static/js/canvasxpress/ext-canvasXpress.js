    ...
    Ext.apply(this, config, {
      ...
      items: {
        ...
        items: [
          {
            title: 'My Shapes',
            html: this.assembleHtml(config.myShapes)
          },
          {
            title: 'Shared Shapes',
            html: this.assembleHtml(config.sharedShapes)
          },
          {
            title: 'Create Shape',
            xtype: 'form',
            labelWidth: 80,
            items: [
              { xtype: 'textfield', fieldLabel: 'Name', allowBlank: false, name:'name', width: 200 },
              { xtype: 'checkbox', hideLabel: true, boxLabel: 'Share with others?', name:'name' },
              { xtype: 'label', text:'Please paste image below:' },
              {
                html: '<iframe width=530 height=300 frameborder=0 id="shapeEditor"></iframe>',
                listeners: {
                  scope: this,
                  activate: function(p) {
                    this.editorDoc = document.getElementById('shapeEditor').contentDocument;
    ...
    if(shapes && shapes.length)
    {
      ...
      for(var i = 0; i < shapes.length;)
      {
        ...
        for(var j = 0; j < 3 && i + j < shapes.length; j++)
          str1.push('<img src="' + shapes[i+j] + '" /><div style="text-align:center;"><button>Use</button><button style="margin-left:10px">Edit</button><button style="margin-left:10px">Remove</button></div>');
        i += j;
...
Ext.canvasXpress.nodeDialog = Ext.extend(Ext.Window, {
  constructor: function(config, extCanvas) {
          ...
          select: function(c, r, i) {
            ...
            if(r.json[0] == 'image' || r.json[0] == 'custom')
            {
              ...
              else
              {
                ...
                {
                  this.value = 'image';
                  var w = new Ext.canvasXpress.customShapeDialog({valField:f0, extCanvas:this});
                  w.show();
                }
              }
            }
    ...
    var bf = function(b, e, noClose) {
      ...
      var p = pf.getValues(), g = gf? gf.getValues() : {};
      for(var i in g)
        p[i] = g[i];
...
//       idMap[id] = l.id;
    ...
    var bbf = function(b, e, noClose) {
      ...
      var p = bf.getValues();
      // assembl parameters
      p.entryid1 = p.id1;//idMap[p.id1];
      p.entryid2 = p.id2;//idMap[p.id2];
